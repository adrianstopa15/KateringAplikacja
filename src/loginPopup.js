...
export default function LoginPopup({ onClose }) {
  ...
  const [usernameValidation, setUsernameValidation] = useState({ loading: false, valid: null, message: '' });
  const [emailValidation, setEmailValidation] = useState({ loading: false, valid: null, message: '' });
  const [password, setPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const [passwordValidation, setPasswordValidation] = useState({ valid: null, message: '' });
  const [passwordsMatch, setPasswordsMatch] = useState({ valid: null, message: '' });
...
//sprawdzanie hasel
  const validatePassword = (newPassword) => {
    ...
    if (!newPassword || newPassword.length < 8 || !hasSpecialChar.test(newPassword)) {
      setPasswordValidation({
        valid: false,
        message: 'Hasło musi zawierać co najmniej 8 znaków oraz 1 znak specjalny.',
      });
    } else {
      setPasswordValidation({ valid: true, message: 'Hasło spełnia wymagania.' });
    }
  };
...
const validateUsername = async (username) => {
  ...
  try {
      // Tutaj wpisz URL do swojego API sprawdzającego dostępność nazwy użytkownika
      const response = await axios.get(`/api/username-check?username=${username}`);
      setUsernameValidation({ loading: false, valid: response.data.isAvailable, message: response.data.isAvailable ? 'Nazwa użytkownika jest dostępna' : 'Nazwa użytkownika jest zajęta' });
    } catch (error) {
      setUsernameValidation({ loading: false, valid: false, message: 'Błąd przy sprawdzaniu nazwy użytkownika' });
    }
  };
// Walidacja formatu e-mail
  const validateEmail = async (email) => {
    ...
    if (!emailRegex.test(email)) {
      setEmailValidation({ loading: false, valid: false, message: 'Niepoprawny format adresu e-mail' });
      return;
    }
    try {
      // Tutaj wpisz URL do swojego API sprawdzającego dostępność e-maila
      const response = await axios.get(`/api/email-check?email=${email}`);
      setEmailValidation({ loading: false, valid: response.data.isAvailable, message: response.data.isAvailable ? 'Adres e-mail jest dostępny' : 'Adres e-mail jest już używany' });
    } catch (error) {
      setEmailValidation({ loading: false, valid: false, message: 'Błąd przy sprawdzaniu adresu e-mail' });
    }
  };
          ...
          <>
            ...
            <form key={formKey} className="login-form">
              ...
              <div className="form-options">
                ...
                <a href="/home" className="forgot-password" onClick={(e) => {
                e.preventDefault(); 
                setIsLogin(false); 
          ...
          <>
            ...
            <form key={formKey} className="login-form">
              ...
              <input
                ...
                required onChange={(e) => validateUsername(e.target.value)}
              />
                  {usernameValidation.loading && <span className="validation-message loading">Sprawdzanie...</span>}
                  {!usernameValidation.loading && usernameValidation.valid !== null && (
                   <span className={`validation-message ${usernameValidation.valid ? 'valid' : 'invalid'}`}>
              ...
              />
                  {emailValidation.loading && <span className="validation-message loading">Sprawdzanie...</span>}
                  {!emailValidation.loading && emailValidation.valid !== null && (
                    <span className={`validation-message ${emailValidation.valid ? 'valid' : 'invalid'}`}>
              ...
              <div className="validation-message invalid">{passwordValidation.message}</div>
              )}
              ...
              {passwordsMatch.valid === false && (
                <div className="validation-message invalid">{passwordsMatch.message}</div>
              )}
              <br></br>
              <ReCAPTCHA
